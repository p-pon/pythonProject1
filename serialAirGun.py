...
min_time_ms     = 50   # Время начала поиска времени запуска(в мс/10)
window_width    = 150  # Окно поиска времени запуска(в мс/10)
# ================================================================================
...
def change_delay(arr):
    ...
    FB1 = arr[:, 2]
    FB2 = arr[:, 5]
    PICK1 = get_ch_delay(FB1)
    PICK2 = get_ch_delay(FB2)
    return PICK1, PICK2
...
def Apply_changes():
    ...
    global INP_DELAY, DELAY1, DELAY2
    INP_DELAY = int(delay.get())
    DELAY1 = INP_DELAY - PICK1
    DELAY2 = INP_DELAY - PICK2
...
tk.Label(root, text='Delay 1ch', state='disabled').grid(row=1, column=12, sticky='s')
tk.Entry(root, width=5, textvariable=del1, state='disabled').grid(row=2, column=12, sticky='s')
tk.Label(root, text='Delay 2ch', state='disabled').grid(row=1, column=13, sticky='s')
...
tk.Label(root, text='Search window,\n length').grid(row=1, column=7, sticky='s')
